...
export function useItemAddition() {
	...
	const removeItem = (item, context) => {
		const currentItems = Array.isArray(context?.items)
			? context.items
			: Array.isArray(context?.invoiceStore?.items)
				? context.invoiceStore.items
				: [];

		const prevLength = currentItems.length;
		const rowId = item?.posa_row_id;
		const matchesItem = (el: any) =>
			el === item ||
			(rowId && String(el?.posa_row_id || "") === String(rowId)) ||
			(!rowId &&
				el?.item_code === item?.item_code &&
				(el?.name === item?.name || el?.item_name === item?.item_name));

		if (context.invoiceStore && rowId) {
			context.invoiceStore.removeItemByRowId(rowId);
		}

		const stillPresent = (list: any[]) =>
			list.findIndex((el) => matchesItem(el)) >= 0;

		const afterPrimaryItems = Array.isArray(context?.items)
			? context.items
			: Array.isArray(context?.invoiceStore?.items)
				? context.invoiceStore.items
				: [];
		...
		if (
			afterPrimaryItems.length === prevLength &&
			stillPresent(afterPrimaryItems)
		) {
			const index = afterPrimaryItems.findIndex((el) => matchesItem(el));
			if (index >= 0) {
				if (
					context.invoiceStore &&
					typeof context.invoiceStore.setItems === "function"
				) {
					const next = [...afterPrimaryItems];
					next.splice(index, 1);
