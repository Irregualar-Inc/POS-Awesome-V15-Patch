...
function parseSalesTypeOptions(raw: unknown): string[] {
...
function requestSalesTypeSelection(
	context: any,
	options: string[],
	currentValue = "",
): Promise<string | null> {
			...
			fields: [
				{
					...
					description: __(
						"Each Sales Type options to show in POS should be on a new line",
					),
				},
			],
			primary_action_label: __("Continue"),
			primary_action: (values: { sales_type?: string }) => {
				const selected = String(values?.sales_type || "").trim();
				if (!selected) {
					context?.toastStore?.show?.({
						title: __("Sales Type is required"),
						color: "error",
					});
					return;
				}
...
export async function show_payment(context: any) {
	...
	try {
		...
		if (
			parseBooleanSetting(
				context?.pos_profile?.posa_allow_select_sales_type,
			)
		) {
			const salesTypeOptions = parseSalesTypeOptions(
				context?.pos_profile?.posa_sales_type_options,
			);

			if (!salesTypeOptions.length) {
				context.toastStore.show({
					title: __(
						"Sales Type options are not configured in POS Profile",
					),
					color: "error",
					detail: __(
						"Add values in Sales Type Options. Each option should be on a new line.",
					),
				});
				return;
			}

			const currentSalesType = String(
				context?.invoice_doc?.sales_type || "",
			).trim();
			const selectedSalesType = await requestSalesTypeSelection(
				context,
				salesTypeOptions,
				currentSalesType,
			);
			...
			if (context?.invoiceStore?.mergeInvoiceDoc) {
				context.invoiceStore.mergeInvoiceDoc({
					sales_type: selectedSalesType,
				});
			}
			context.invoice_doc = Object.assign({}, context.invoice_doc || {}, {
				sales_type: selectedSalesType,
			});
		}
