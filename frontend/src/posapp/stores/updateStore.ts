...
function isLocalDevelopmentHost(): boolean {
	if (!hasBrowserContext || !window.location) {
		return false;
	}
...
function forceReloadPosApp() {
	...
	try {
		...
		console.warn("Failed to clear update recovery keys before reload", err);
	}

	window.location.replace(`/app/posapp?_posa_force_reload=${Date.now()}`);
}
	...
	actions: {
		...
		async checkForUpdates(force = false) {
			...
			if (isLocalDevelopmentHost()) {
				...
				this.availableTimestamp = this.currentVersion
					? parseTimestamp(this.currentVersion)
					: null;
				this.availableMessage = null;
				this.availableCommit = null;
				this.availableCommitDate = null;
				this.availableBranch = null;
			...
			try {
				...
				let targetVersion = buildVersion || null;
				let targetTimestamp: number | null = null;
				let targetMessage: string | null = null;
				let targetCommit: string | null = null;
				let targetCommitDate: string | null = null;
				let targetBranch: string | null = null;
				let targetCommits: UpdateState["availableCommits"] = [];
				if (currentCommit) {
				...
				if (r?.message?.remote_ahead) {
					...
					if (branch) {
						...
						targetBranch = r.message.remote_sample_branch || branch;
						targetCommit =
							sample.commit_hash || remoteAhead?.[branch] || null;
						targetMessage = sample.commit_message?.trim() || null;
						targetCommitDate = sample.commit_date || null;
						targetCommits = r?.message?.remote_commits || [];
						// Keep prompt identity tied to build version to avoid repeated re-prompts
						// when remote commit metadata changes without a new built asset.
						targetVersion = buildVersion || targetCommit || null;
					}
				}
				if (!targetMessage && r?.message?.commit_message) {
					targetMessage = r?.message?.commit_message?.trim() || null;
					targetCommit = r?.message?.commit_hash || null;
					targetCommitDate = r?.message?.commit_date || null;
				}

				if (targetVersion) {
					this.setAvailableVersion(targetVersion, targetTimestamp);
				}
				this.availableMessage = targetMessage;
				this.availableCommit = targetCommit;
				this.availableCommitDate = targetCommitDate;
				this.availableBranch = targetBranch;
				this.availableCommits = targetCommits;
			} catch (err) {
		...
		reloadNow() {
			...
			if (typeof this.reloadAction === "function") {
				try {
					...
					console.warn(
						"Reload action failed, forcing hard reload",
						err,
					);
				}
			}
